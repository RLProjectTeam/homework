...
def run_selfplay_episode(selfplay, alice, bob, optimisers_alice, optimisers_bob, reward_scale=1.0, tMax=80):
    ...
    while True:
        tA += 1
        observation = selfplay.alice_observe()   ########### s_t, s_0
        
        ############## add ####################
        if set_middle==False:
            set_middle, alice_middle_state = determine_this_state_as_middle(observation)
        #######################################
    ...
    add_reward = 0
    while True:
        observation = selfplay.bob_observe() ####### s_t, s* (alice end)
